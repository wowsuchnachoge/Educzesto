...
<?php  
	...
	$usuario = new Usuario();
	$nombreAlumnoBitacora = new Usuario();
	...
	$nombreAlumnoBitacora->consultaUsuarioId($_POST["idUsuario"]);
	...
	$arregloBitacora = $bitacora->consultaBitacoraAlumnosId($_POST["idUsuario"]);

	$usuario->cierraBaseDatos();
	//$nombreAlumnoBitacora->cierraBaseDatos();
...
<body>
	...
	<main>
		<div class="container mt-3">
			<h3><i class="icon-user text-dark mr-2"></i><?php  echo "Bitácora";?></h3>
		</div>
		<div class="row">
			<div class="col-8">
				<ul id="listPostit">
					...
					<li id="postit">
						<a>
							...
							<p class="text-danger buttonEliminaBitacoraHistorico no-margin text-center" data-id_bitacora="<?php  echo $valor['idBitacora'];?>" style="font-size: xx-small;">Eliminar <i class="icon-trash text-danger"></i></p>												
							<hr class="no-margin">
							...
							<form action="php/sql/controladores/cntRegistroBitacoraRealizado.php" method="POST">																				
								...
								<button onclick="showGuardado()"type="submit" class="btn btn-dark btn-sm float-right mt-2 buttonCambiarEstadoBitacoraAlumnos" style="font-size: xx-small; margin-left: 5px;" data-id_bitacora="<?php echo $valor["idBitacora"];?>" data-estado_bitacora="<?php echo $valor["realizado"];?>"><i class="icon-floppy text-white"></i>Guardar</button>					
								<input type="text" name="idBitacora" value="<?php echo $idABitacora;?>" hidden>																									
							</form>							
						</a>					
					</li>
					<?}?>				
				...
				<div class="row d-flex justify-content-center align-items-center">
					<div class="col-md-8">
						...
						<form id="regForm" action="php/sql/controladores/cntNuevoElementoBitacoraAlumnos.php" method="POST">
							<h3 id="register"><i class="bi bi-plus-circle mr-2"></i> Agregar a su bitacora</h3>
							...
							<div class="tab">
								<h6>Ingresa la entrada a su bitácora:</h6>
								...
								<p> <input type="text" name="idUsuario" value="<?php echo $_POST["idUsuario"];?>" style="display: none;"> </p>
